...
def main(args):
    ...
    #여기서부터 새로 작성
    # PATH='C:/Users/hyeyoung/PycharmProjects/test/koreanmult/korean_audiotext_transformer'
    # #torch.save(model.state_dict(),PATH)
    # torch.save(model,PATH)
    #model=torch.load(PATH)
    #model.load_state_dict(torch.load(PATH))
    #----------
    #model.load_state_dict(torch.load(args.model_path)) #이게 original
    #model.load_state_dict(torch.load(args.model_path), strict=False)
    #model.load_state_dict(torch.load(args.model_path), strict=False)
    #model.load_state_dict(torch.load(args.model_path),strict=False)
    #model.load_state_dict(torch.load(args.model_path), map_location=device_)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--only_audio', action='store_false')
    #parser.add_argument('--only_text', action='store_true')
    parser.add_argument('--only_text', action='store_false')
    ...
    #parser.add_argument('--model_path', type=str, default='./practice/epoch1-loss1.9628-f10.0995.bin')
