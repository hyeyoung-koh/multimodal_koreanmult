...
def extract_audio(path, df):
    src_files = [
        os.path.join(path, folder, file, files)
        for folder in os.listdir(path)
        if os.path.isdir(os.path.join(path, folder))
        for file in os.listdir(os.path.join(path, folder))
        if (os.path.isdir(os.path.join(path, folder,file))) and (file=='movie')#
        for files in os.listdir(os.path.join(path, folder, file))]
# def extract_audio(path, df):
#     src_files = [
...
#         for file in os.listdir(os.path.join(path, folder))
#     ]
    # ./korean_audiotext_transformer/mydataset/

    # src_files = [
    #     os.path.join(
    #         path, "str(i)/movie") for i in ["000","001","002","003","004","005","006","007","008","009","010","011","012"]
    # ]
    columns = ['person_idx', 'audio', 'sentence', 'emotion'] #데이터프레임의 컬럼명
    new_df = pd.DataFrame(columns=columns)
    cur_idxs = list(df.index)
    for src_file in tqdm(src_files, total=len(src_files)):
        # find sentence & emotion
        #splitted=src_file.split('')[-1]
        splitted = src_file.split('\\')[-1].split('.')[0].split('-')
        #person_idx=src_file.split('-')[-1].split()
        #sent_idx=src_file.split('-')[-1].split()
        #splitted = src_file.split('-')[-1].split('-')[0].split('.')[0]
        #person_idx=int(src_file.split('-')[0])
        #sent_idx=int(src_file.split('-')[-1])
        #splitted=src_file.split('-')
        #splitted = src_file.split('/')[-1].split('-')
        person_idx, sent_idx = map(int, splitted)
        #person_idx,sent_idx=map(float,splitted)
        #person_idx,sent_idx=map(int,splitted)
